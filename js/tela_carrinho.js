...
const montaTelaCarrinho = () => {
    ...
    itens.forEach((elem, i) => {
        ...
        inputQuantidade.addEventListener('input', () => {
            ...
            if (!Number.isInteger(quantidade) || quantidade <= 0) {             // ALTERAÇÃO PELO GPT
            ...
            atualizaValorItem(pCalc, elem.valor_unitario, quantidade) //atualiza o valor do item + a quantidade do item
        ...
        pCalc.innerHTML = `R$ ${(elem.valor_unitario * elem.quantidade).toFixed(2).replace('.', ',')}` //Exibe o valor + quantidade do produto
...
const atualizaValorItem = (elemento, preco, quantidade) => {
    ...
    elemento.innerHTML =
        `R$ ${ (preco * quantidade) .toFixed(2).replace('.', ',')}`            // ALTERAÇÃO PELO GPT

}

montaTelaCarrinho() //atualiza a pagina
...
atualizaResumo() //atualiza o resumo
